...
func (tr *pgEventsRepository) SearchEvents(p domain.SearchEventsInput) (*[]domain.Event, error) {
	...
	query := fmt.Sprintf("SELECT event_id, name, project_id, timestamp from events %s", where)
	event := &[]domain.Event{}
...
func (tr *pgEventsRepository) ReadEvent(id int) (*domain.Event, error) {
	query := "SELECT event_id, name, project_id, timestamp from events WHERE event_id = $1"
	event := &domain.Event{}
...
func (tr *pgEventsRepository) CreateEvent(event domain.Event) (int, error) {
	query := `INSERT INTO events (name, project_id) VALUES ($1, $2) RETURNING event_id`
	var id int
	if err := tr.conn.QueryRow(query, event.Name, event.ProjectID).Scan(&id); err != nil {
		log.Errorf("query error: %v", err)
		return -1, err
	}
	return id, nil
